...
function LayoutThumbnail({ layout, active, isSpread }: { layout: PageLayout; active: boolean; isSpread?: boolean }) {
  ...
  return (
    <svg viewBox={`0 0 ${vw} 100`} className="w-full h-full">
      {isSpread && (
        <>
          <rect x="0" y="0" width="98" height="100" fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.10)" strokeWidth="1" rx="1" />
          <rect x="102" y="0" width="98" height="100" fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.10)" strokeWidth="1" rx="1" />
          <line x1="100" y1="0" x2="100" y2="100" stroke="rgba(0,0,0,0.4)" strokeWidth="2" />
        </>
      )}
      {!isSpread && (
        <rect x="0" y="0" width="100" height="100" fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.12)" strokeWidth="1.5" rx="1" />
      )}
      {layout.slots.map((slot) => {
        ...
        return (
          <rect
            ...
            fill={
              active
                ? slot.type === 'image' ? 'rgba(249,115,22,0.35)' : 'rgba(99,140,255,0.35)'
                : slot.type === 'image' ? 'rgba(255,255,255,0.13)' : 'rgba(99,140,255,0.18)'
            }
            stroke={
              active
                ? slot.type === 'image' ? 'rgba(249,115,22,0.8)' : 'rgba(99,140,255,0.8)'
                : slot.type === 'image' ? 'rgba(255,255,255,0.25)' : 'rgba(99,140,255,0.45)'
            }
            strokeWidth="1.5"
            rx="0.5"
          />
      ...
      {(layout.id === 'custom' || layout.id === 'spread-custom' || layout.id === 'zine-custom') && (
        <text x={vw / 2} y="54" textAnchor="middle" fontSize="18" fill="rgba(255,255,255,0.25)" fontFamily="monospace">+</text>
      )}
    </svg>
  )
}
...
function SlotRow({ slot, onUpdate, onDelete }: { slot: BookSlotType; onUpdate: (s: BookSlotType) => void; onDelete: () => void }) {
  ...
  return (
    <div className="bg-black/30 border border-white/[0.08] rounded-sm p-2 space-y-2">
      <div className="flex items-center justify-between">
        <span className={`text-[8px] uppercase font-bold tracking-wider ${slot.type === 'image' ? 'text-orange-400/80' : 'text-blue-400/80'}`}>
          {slot.type}
        </span>
        <button onClick={onDelete} className="text-[10px] text-white/30 hover:text-red-400 transition-colors">×</button>
      </div>
        ...
        {fields.map(({ key, label }) => (
          <div key={key} className="flex flex-col items-center gap-0.5">
            ...
            <input
              ...
              className="w-full text-[8px] bg-transparent border-b border-white/15 text-gray-300 text-center focus:outline-none focus:border-orange-500 tabular-nums"
            />
...
export function BookPanel() {
  ...
  const inactiveBtn = 'text-white/50 hover:text-white/75 border-white/10 hover:border-white/20'
  const activeBookBtn = 'bg-orange-500/15 text-orange-300 border-orange-500/35'
  const activeZineBtn = 'bg-purple-500/15 text-purple-300 border-purple-500/40'

  return (
    <div
      className="flex flex-col h-full z-20"
      style={{ width: 272, minWidth: 272, borderLeft: '1px solid rgba(255,255,255,0.07)', background: 'rgba(10,9,8,0.98)' }}
    >
      {/* Book / Zine mode */}
      <div className="px-3 pt-3 pb-2 border-b border-white/[0.06]">
        <div className="flex gap-1">
          {(['book', 'zine'] as const).map((style) => (
            <button
              ...
              className={`flex-1 py-2 text-[9px] uppercase tracking-[0.2em] rounded-sm border transition-all font-bold ${
                bookStyle === style
                  ? style === 'zine' ? activeZineBtn : activeBookBtn
                  : inactiveBtn
              }`}
            >
        ...
        {bookStyle === 'zine' && (
          <p className="mt-2 text-[7px] text-purple-300/55 leading-relaxed">
      ...
      <div className="px-4 pt-4 pb-3 border-b border-white/[0.06]">
        <label className="text-[8px] text-white/50 uppercase tracking-[0.25em] block mb-2 font-bold">Book Size</label>
        <select
          value={sizeId}
          onChange={(e) => setBookSizeId(e.target.value)}
          className="w-full text-[10px] bg-black/50 border border-white/[0.12] text-gray-200 rounded-sm px-2 py-2 focus:outline-none focus:border-orange-500/50"
        >
          {bookStyle === 'zine' && (
            <optgroup label="Zine">
        ...
        {sizeId === 'custom' ? (
          <div className="mt-2 flex items-center gap-2">
            <input
              ...
              className="w-16 text-xs bg-transparent border-b border-white/20 text-orange-300 py-0.5 text-right tabular-nums focus:outline-none focus:border-orange-500"
            />
            ...
            <input
              ...
              className="w-16 text-xs bg-transparent border-b border-white/20 text-orange-300 py-0.5 text-right tabular-nums focus:outline-none focus:border-orange-500"
            />
            <span className="text-[9px] text-white/40">in</span>
          </div>
        ) : (
          <div className="mt-1.5 text-[8px] text-white/35 font-mono">{size.width} × {size.height} {size.unit}</div>
        )}
      </div>

      {/* Page count */}
      <div className="px-4 py-2.5 border-b border-white/[0.06] flex items-center justify-between">
        <span className="text-[8px] text-white/50 uppercase tracking-[0.2em] font-bold">Pages</span>
        <input
          type="number" min={1} max={200} value={numPages}
          onChange={(e) => setBookNumPages(parseInt(e.target.value) || 1)}
          className="w-14 text-xs bg-transparent border-b border-white/15 text-orange-300 py-0.5 text-right tabular-nums focus:outline-none focus:border-orange-500"
        />
      </div>
          ...
          {(['page', 'spread'] as const).map((mode) => (
            <button
              ...
              className={`flex-1 py-1.5 text-[8px] uppercase tracking-wider rounded-sm border transition-all ${
                layoutMode === mode ? activeBookBtn : inactiveBtn
              }`}
            >
        ...
        <>
              ...
              {(['left', 'right'] as const).map((t) => {
                ...
                return (
                  <button
                    ...
                    disabled={!valid}
                    className={`flex-1 py-1.5 text-[8px] uppercase tracking-wider rounded-sm border transition-all ${
                      target === t ? activeBookBtn : inactiveBtn
          ...
          {showPageCustomEditor ? (
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-[8px] text-white/50 uppercase tracking-[0.25em] font-bold">Custom Layout</span>
                <button
                  onClick={() => applyPageLayout('full-bleed')}
                  className="text-[8px] text-white/45 hover:text-white/75 uppercase tracking-wider transition-colors"
                >
                  ← Back
                </button>
              </div>
              <p className="text-[8px] text-white/40 leading-relaxed">
                Add slots, then set position and size as percentages of the page.
              </p>
              <div className="flex gap-1.5">
                <button
                  onClick={() => addCustomPageSlot('image')}
                  className="flex-1 py-2 text-[8px] uppercase tracking-wide border border-orange-500/40 text-orange-300/80 hover:bg-orange-500/15 rounded-sm transition-colors font-bold"
                >
                  + Image Slot
                </button>
                <button
                  onClick={() => addCustomPageSlot('text')}
                  className="flex-1 py-2 text-[8px] uppercase tracking-wide border border-blue-500/40 text-blue-300/80 hover:bg-blue-500/15 rounded-sm transition-colors font-bold"
                >
                  + Text Slot
                </button>
              </div>
              {editablePageSlots.length === 0 && (
                <div className="py-6 text-center">
                  <p className="text-[9px] text-white/30 italic">No slots yet — add one above.</p>
                </div>
              )}
            ...
            <div className="flex-1 overflow-y-auto p-3">
              <div className="text-[8px] text-white/50 uppercase tracking-[0.25em] mb-3 font-bold">Layout</div>
              <div className="grid grid-cols-3 gap-2">
                {(bookStyle === 'zine' ? ZINE_LAYOUTS : PAGE_LAYOUTS).map((layout) => {
                  const active = activePageLayoutId === layout.id
                  const accentActive = bookStyle === 'zine' ? activeZineBtn : activeBookBtn
                  const accentText = bookStyle === 'zine' ? 'text-purple-300/90' : 'text-orange-300/90'
                  return (
                    <button
                      key={layout.id}
                      onClick={() => applyPageLayout(layout.id)}
                      className={`flex flex-col items-center gap-1.5 p-1.5 rounded-sm border transition-all ${
                        active ? accentActive : 'border-white/[0.08] hover:border-white/20 bg-white/[0.02] hover:bg-white/[0.04]'
                      }`}
                    >
                      <div className="w-full aspect-square">
                        <LayoutThumbnail layout={layout} active={active} />
                      </div>
                      <span className={`text-[7px] uppercase tracking-wide text-center leading-tight ${active ? accentText : 'text-white/45'}`}>
                        {layout.name}
                      </span>
                    </button>
                  )
      ...
      {layoutMode === 'spread' && (
        <div className="flex-1 overflow-y-auto p-3">
          <div className="text-[8px] text-white/50 uppercase tracking-[0.2em] mb-3 font-bold">
            Spread Layout — Spread {currentSpreadIndex + 1}
          </div>
          {activeSpreadLayoutId && (
            <button
              onClick={() => setSpreadLayout(currentSpreadIndex, null)}
              className="w-full mb-3 py-1.5 text-[8px] uppercase tracking-wide border border-white/15 text-white/50 hover:text-white/75 hover:border-white/30 rounded-sm transition-colors"
            >
              ✕ Clear Spread Layout
            </button>
          )}
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-1.5 p-2 rounded-sm border transition-all ${
                    active ? activeBookBtn : 'border-white/[0.08] hover:border-white/20 bg-white/[0.02] hover:bg-white/[0.04]'
                  }`}
                >
                  <div className="w-full" style={{ aspectRatio: '2/1' }}>
                    <LayoutThumbnail layout={layout} active={active} isSpread />
                  </div>
                  <span className={`text-[7px] uppercase tracking-wide text-center leading-tight ${active ? 'text-orange-300/90' : 'text-white/45'}`}>
                    {layout.name}
                  </span>
                </button>
              )
          ...
          {showSpreadCustomEditor && (
            <div className="mt-4 space-y-2">
              ...
              <div className="text-[8px] text-white/50 uppercase tracking-[0.2em] pt-1">Freeform Spread Slots</div>
              <div className="flex gap-1.5">
                <button onClick={() => addCustomSpreadSlot('image')} className="flex-1 py-1.5 text-[8px] uppercase tracking-wide border border-orange-500/35 text-orange-300/70 hover:bg-orange-500/12 rounded-sm transition-colors">+ Image</button>
                <button onClick={() => addCustomSpreadSlot('text')} className="flex-1 py-1.5 text-[8px] uppercase tracking-wide border border-blue-500/35 text-blue-300/70 hover:bg-blue-500/12 rounded-sm transition-colors">+ Text</button>
              </div>
              {editableSpreadSlots.length === 0 && <p className="text-[8px] text-white/35 italic text-center py-2">No slots yet.</p>}
      ...
      {/* Export */}
      <div className="px-4 py-4 border-t border-white/[0.08] space-y-2 bg-black/40">
        {/* DPI selector */}
        <div className="flex gap-1 items-center">
          <span className="text-[7px] text-white/40 uppercase tracking-wider mr-1">DPI</span>
            ...
            <button
              ...
              className={`flex-1 py-1.5 text-[8px] font-mono rounded-sm border transition-all ${
                pdfDpi === dpi
                  ...
                  : 'text-white/45 border-white/10 hover:text-white/70 hover:border-white/22'
              } disabled:opacity-30`}
            >
              {dpi}
            </button>
        ...
        <button
          ...
          disabled={!!pdfProgress}
          className="w-full py-2.5 text-[9px] font-bold uppercase tracking-[0.2em] rounded-sm border border-orange-500/40 bg-orange-500/15 text-orange-200 hover:bg-orange-500/25 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {pdfProgress
        ...
        <button
          ...
          className="w-full py-1.5 text-[8px] text-white/40 hover:text-white/65 uppercase tracking-[0.25em] transition-colors"
        >
          Export JSON (InDesign)
        </button>
        <button
          onClick={() => { if (window.confirm('Clear all photo assignments?')) clearAllAssignments() }}
          className="w-full py-1 text-[8px] text-white/25 hover:text-red-400 uppercase tracking-[0.25em] transition-colors font-bold"
        >
          Reset
        </button>
      </div>
